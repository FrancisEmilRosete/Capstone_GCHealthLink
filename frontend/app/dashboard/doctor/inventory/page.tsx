...
const EXPIRY_STYLE: Record<string, string> = {
  ...
  'EXPIRING_SOON': 'bg-yellow-50 text-yellow-600 border-yellow-100',
...
function downloadInventoryCsv(items: InventoryItem[]) {
  const headers = ['Item Name', 'Current Stock', 'Reorder Threshold', 'Unit', 'Status', 'Expiration Date', 'Expiry Status'];
    ...
    >
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            <label className="text-xs font-semibold text-gray-700 block mb-1">Expiration Date (Optional)</label>
            <input
              type="date"
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-gray-100 text-gray-400">
                ...
                {!isDoctor && <th className="text-right px-4 py-3 font-semibold">Actions</th>}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                    ...
                    <td className="px-4 py-3">
                      {item.expirationDate ? (
                        <div className="flex flex-col gap-1">
                          <span className="text-[11px] text-gray-600 font-medium">{new Date(item.expirationDate).toLocaleDateString()}</span>
                          <span className={`text-[10px] w-max font-semibold border px-2 py-0.5 rounded-md ${item.expiryStatus ? EXPIRY_STYLE[item.expiryStatus] : ''}`}>
                            {item.expiryStatus ? item.expiryStatus.replace('_', ' ') : 'N/A'}
                          </span>
                        </div>
                      ) : (
