...
function mapToTimelineEvents(student: StudentProfile | null): MedicalTrackingEvent[] {
  ...
  for (const appointment of student.appointments || []) {
    ...
    events.push({
      ...
      dateIso,
      title: normalizeLabel(appointment.serviceType, 'Clinic appointment'),
  ...
  useEffect(() => {
    async function loadProfile() {
      ...
      try {
        ...
        setError(err instanceof ApiError ? err.message : 'Failed to load student history.');
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="p-5 space-y-5 max-w-5xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <Link
          href={`/dashboard/staff/record/${encodeURIComponent(studentNumber)}`}
